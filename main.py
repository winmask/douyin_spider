...
class Task(object):
    ...
    def __init__(self,sec_user_id,count):
    ...
    def task(self):
        url='https://www.douyin.com/aweme/v1/web/aweme/post/?device_platform=webapp&aid=6383&channel=channel_pc_web&sec_user_id=' + self.sec_user_id + '&max_cursor=' + str(self.max_cursor) + '&locate_query=false&show_live_replay_strategy=1&count=200&publish_video_strategy_type=2&pc_client_type=1&version_code=170400&version_name=17.4.0&cookie_enabled=true&screen_width=1536&screen_height=864&browser_language=zh-CN&browser_platform=Win32&browser_name=Chrome&browser_version=108.0.5359.95&browser_online=true&engine_name=Blink&engine_version=108.0.5359.95&os_name=Windows&os_version=10&cpu_core_num=8&device_memory=8&platform=PC&downlink=10&effective_type=4g&round_trip_time=250'
        headers = {
        ...
        if self.numb==0:
            try:
                mkdir(resp["aweme_list"][0]["author"]["nickname"]+"/")
                mkdir(resp["aweme_list"][0]["author"]["nickname"] + "/video/")
                mkdir(resp["aweme_list"][0]["author"]["nickname"] + "/picture/")
                print("首次创建{}缓存文件夹".format(resp["aweme_list"][0]["author"]["nickname"]))
            except:
                print("{}缓存文件夹已存在".format(resp["aweme_list"][0]["author"]["nickname"]))
            with open(resp["aweme_list"][0]["author"]["nickname"]+"/"+resp["aweme_list"][0]["author"]["nickname"]+"_采集数据.csv",'w',newline='',encoding='gbk',errors='ignore') as csvfile:
        ...
        for aweme in resp["aweme_list"]:
            ...
            with open(resp["aweme_list"][0]["author"]["nickname"]+"/"+resp["aweme_list"][0]["author"]["nickname"]+"_采集数据.csv",'a',newline='',encoding='gbk',errors='ignore') as csvfile:
                fieldnames=['aweme_id','时间','title','格式','收藏','评论','点赞','分享','share_url']
                writer=csv.DictWriter(csvfile,fieldnames=fieldnames)
            ...
            if aweme['images']==None:
                ...
                for i in url_list:
                    ...
                    print(url)
                    video = get(url)
                    with open(aweme["author"]["nickname"] + "/picture/" + aweme["aweme_id"] + '_'+str(s)+'.jpeg', 'wb') as f:
                        f.write(video.content)
                    self.picture+=1
...
def main():
    print("github开源地址:https://github.com/raindrop-hb\n使用请保留版权\n欢迎使用raindrop抖音爬虫_解析工具")
    a=input('输入主页链接：')
    ...
    if "?previous_page=app_code_link" in a:
        a = findall('https://www.douyin.com/user/(.*)\?previous_page=app_code_link', a)[0]
    c=Task(a,b)
    c.run()
